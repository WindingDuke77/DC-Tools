...
export default function CableGenerator() {
  ...
  const handleCopySwatch = (key, hex) => {
    navigator.clipboard.writeText(hex.toUpperCase())
    setCopiedSwatch(key)
    setTimeout(() => setCopiedSwatch(k => k === key ? null : k), 1200)
  }
              ...
              {saved.map((entry, index) => (
                ...
                >
                  ...
                  <div className="flex gap-1 shrink-0">
                    <button
                      onClick={() => handleCopySwatch(`${entry.id}-rack`, entry.rack.color)}
                      className="w-5 h-5 rounded transition-transform hover:scale-110 cursor-pointer"
                      style={{
                        backgroundColor: entry.rack.color,
                        outline: copiedSwatch === `${entry.id}-rack` ? '2px solid white' : '2px solid transparent',
                        outlineOffset: '1px',
                      }}
                      title={copiedSwatch === `${entry.id}-rack` ? 'Copied!' : `Rack: ${entry.rack.color.toUpperCase()}`}
                    />
                    {entry.cables.map((c, i) => (
                      <button
                        key={i}
                        onClick={() => handleCopySwatch(`${entry.id}-cable-${i}`, c.color)}
                        className="w-5 h-5 rounded transition-transform hover:scale-110 cursor-pointer"
                        style={{
                          backgroundColor: c.color,
                          outline: copiedSwatch === `${entry.id}-cable-${i}` ? '2px solid white' : '2px solid transparent',
                          outlineOffset: '1px',
                        }}
                        title={copiedSwatch === `${entry.id}-cable-${i}` ? 'Copied!' : `Cable ${String.fromCharCode(65 + i)}: ${c.color.toUpperCase()}`}
                      />
                    ))}
                  </div>
